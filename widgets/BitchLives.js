...
async function getVideos(params = {}) {
  try {
    if (!params.sort_by) throw new Error();
    ...
    if (!response?.data?.zhubo || !Array.isArray(response.data.zhubo)) throw new Error();
    ...
    return [{ id: "err", type: "text", title: "视频获取失败，请下拉重试或检查网络" }];
  }
}
