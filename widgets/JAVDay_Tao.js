...
async function loadDetail(link) {
  ...
  if (dplayerScript) {
    ...
    resolvedVideoUrl = extractVideoUrlFromDPlayerScript(scriptContent);
  }
  
  if (!resolvedVideoUrl) {
    resolvedVideoUrl = $("video#J_prismPlayer").attr("src") || 
  ...
  if (!resolvedVideoUrl) {
    ...
    if (scriptSources) {
      ...
      if (m3u8Match && m3u8Match[1]) {
        resolvedVideoUrl = m3u8Match[1];
      }
    }
  }

  if (!resolvedVideoUrl) {
    resolvedVideoUrl = $("video[src]").attr("src") || 
  ...
  $(".videoBox").each((index, element) => {
    const $item = $(element);
    let itemLink = $item.attr("href");
    const title = $item.find(".videoBox-info .title").text().trim();
    const imgSrc = getCoverImgSrc($item);

    if (!itemLink || !title) return;
    
    if (!itemLink.startsWith("http")) {
      itemLink = itemLink.startsWith("//") 
        ? `https:${itemLink}`
        : `https://javday.app${itemLink.startsWith("/") ? "" : "/"}${itemLink}`;
    }

    itemLink = itemLink.replace(/([^:]\/)\/+/g, '$1');
