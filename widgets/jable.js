...
async function parseHtml(htmlContent) {
  ...
  const sectionSelector = ".site-content .py-3,.pb-e-lg-40,.container,body";
    ...
    if (itemElements && itemElements.length > 0) {
      for (const itemElement of itemElements) {
        ...
        if (url && url.includes("jable.tv")) {
          ...
          const cover = coverId.attr("data-src") || coverId.attr("data-original") || coverId.attr("src") || "";
          const video = coverId.attr("data-preview") || coverId.attr("data-mediabook") || "";
...
async function loadDetail(link) {
  ...
  let hlsUrl = "";
  const match = String(response.data || "").match(/var\s+hlsUrl\s*=\s*['"](.*?)['"]/i);
  if (match && match[1]) hlsUrl = match[1];
  if (!hlsUrl) {
