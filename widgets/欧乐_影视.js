// @name 欧乐影视
// @description 欧乐影视 - 支持自定义API地址、智能匹配、全集返回；新增分类浏览（电影/剧集/综艺/动漫/短剧）
// @version 2.1.3

var DEFAULT_API_HOST = "https://api.olelive.com";
var REFERER = "https://www.olelive.com";
var REQUEST_TIMEOUT = 10000;
var MIN_MATCH_SCORE = 70;
var MAX_RETRIES = 2;
  ...
  return new Promise(function(resolve, reject) {
    Widget.http.get(url, {
      ...
      if (retryCount < MAX_RETRIES) {
        ...
        setTimeout(function() {
          httpGet(url, retryCount + 1).then(resolve).catch(reject);
        }, 1000);
      } else {
...
// ==================== 原有搜索和详情函数 ====================
function searchVod(apiHost, keyword, pg) {
  pg = pg || 1;
  ...
  return httpGet(url).then(function(res) {
    ...
    if (res.code !== 0) {
      logInfo("搜索API返回异常码 " + res.code + ": " + JSON.stringify(res));
      return [];
    }
    if (!res.data || !res.data.data) {
      logInfo("搜索返回缺少 data.data 字段");
      return [];
    }
    var vodData = null;
    for (var i = 0; i < res.data.data.length; i++) {
      if (res.data.data[i].type === "vod") {
        vodData = res.data.data[i];
        break;
      }
    }
    if (!vodData || !vodData.list) {
      logInfo("未找到 vod 类型数据或 list 为空");
      return [];
    }
    var results = [];
...
function getDetail(apiHost, vodId) {
  var url = buildApiUrl(apiHost, "/v1/pub/vod/detail/" + vodId + "/true", {});
  logInfo("详情URL: " + url);
  return httpGet(url).then(function(res) {
    if (!res || res.code !== 0) {
      logInfo("详情API返回异常: " + JSON.stringify(res));
      return null;
    }
    return res.data;
  }).catch(function(e) {
...
function calculateMatchScore(searchName, itemName, year) {
  var cleanSearch = cleanTitle(searchName);
  var cleanItem = cleanTitle(itemName);
  if (cleanItem === cleanSearch) return 100;
  if (cleanItem.indexOf(cleanSearch) !== -1 && cleanSearch.length >= 3) return 80;
  if (cleanSearch.indexOf(cleanItem) !== -1 && cleanItem.length >= 3) return 70;
  var matchCount = 0;
  for (var i = 0; i < cleanSearch.length; i++) {
    if (cleanItem.indexOf(cleanSearch[i]) !== -1) matchCount++;
  }
  var score = (matchCount / cleanSearch.length) * 60;
  if (year && searchName.indexOf(year) !== -1) score += 10;
  return Math.min(score, 99);
}

// ==================== 原有 loadResource（完全保留） ====================
function loadResource(params) {
  ...
  if (!seriesName && params && params.TestTitle) {
  ...
  return searchVod(apiHost, searchKeyword).then(function(searchResults) {
    if (!searchResults.length) {
      logInfo("未找到相关视频: " + searchKeyword);
      return [];
    }
    var cleanSearchName = cleanTitle(searchKeyword);
    var matchedItem = null;
    var matchScore = 0;
    for (var i = 0; i < searchResults.length; i++) {
      var item = searchResults[i];
      var score = calculateMatchScore(cleanSearchName, item.vod_name, item.year);
    ...
    return getDetail(apiHost, matchedItem.vod_id).then(function(detail) {
      if (!detail || !detail.urls || !detail.urls.length) {
        logInfo("获取详情失败或无播放源");
        return [];
      }
      var realTitle = detail.title || detail.name || matchedItem.vod_name;
      ...
      for (var i = 0; i < detail.urls.length; i++) {
        ...
        var language = extractLanguage(matchedItem.vod_remarks, epName);
        if (type === "movie") {
          if (matchedResources.length === 0) {
            matchedResources.push({ url: item.url, title: item.title, epNum: epNum, language: language });
          }
        } else {
          if (episode !== null) {
            if (epNum === episode) {
      ...
      for (var i = 0; i < matchedResources.length; i++) {
        ...
        if (urlSet.has(videoUrl)) continue;
        urlSet.add(videoUrl);
        var description = realTitle;
        var epName = item.title || "";
        if (type === "tv" && epName && epName.indexOf("正片") === -1) {
        ...
        uniqueResources.push({
          id: matchedItem.vod_id + "_" + Date.now() + "_" + uniqueResources.length,
          name: "欧乐影视",
          type: type,
...
function fetchCategoryList(apiHost, cateId, area, sortBy, page) {
  ...
  return httpGet(url).then(function(res) {
    ...
    for (var i = 0; i < list.length; i++) {
      ...
      var description = displayYear + " · " + categoryName;
      items.push({
        id: "ole_" + item.id,
        type: "url",
        title: item.name,
...
function loadMovieList(params) {
  ...
  return fetchCategoryList(apiHost, CATEGORY_ID.movie, area, sortValue, page).then(function(items) {
...
function loadTvList(params) {
  ...
  var page = (params && params.page) ? parseInt(params.page) : 1;
  var sortValue = SORT_MAP[sortBy] || "hot";
  return fetchCategoryList(apiHost, CATEGORY_ID.tv, area, sortValue, page).then(function(items) {
    if (items.length === 0 && page === 1) {
      return [{ id: "empty", type: "text", title: "暂无数据，请检查网络或API地址" }];
    }
    return items;
...
function loadVarietyList(params) {
  ...
  var page = (params && params.page) ? parseInt(params.page) : 1;
  var sortValue = SORT_MAP[sortBy] || "hot";
  return fetchCategoryList(apiHost, CATEGORY_ID.variety, area, sortValue, page).then(function(items) {
    if (items.length === 0 && page === 1) {
      return [{ id: "empty", type: "text", title: "暂无数据，请检查网络或API地址" }];
    }
    return items;
...
function loadAnimeList(params) {
  ...
  var page = (params && params.page) ? parseInt(params.page) : 1;
  var sortValue = SORT_MAP[sortBy] || "hot";
  return fetchCategoryList(apiHost, CATEGORY_ID.anime, area, sortValue, page).then(function(items) {
    if (items.length === 0 && page === 1) {
      return [{ id: "empty", type: "text", title: "暂无数据，请检查网络或API地址" }];
    }
    return items;
...
function loadShortList(params) {
  ...
  var page = (params && params.page) ? parseInt(params.page) : 1;
  var sortValue = SORT_MAP[sortBy] || "hot";
  return fetchCategoryList(apiHost, CATEGORY_ID.short, area, sortValue, page).then(function(items) {
    if (items.length === 0 && page === 1) {
      return [{ id: "empty", type: "text", title: "暂无数据，请检查网络或API地址" }];
    }
    return items;
...
function loadDetail(params) {
  ...
  if (typeof params === "object") {
    vodId = params.vod_id || params.id;
    apiHost = params.api_host || params.ApiHost;
    if (vodId && typeof vodId === "string" && vodId.indexOf("ole_") === 0) {
      vodId = vodId.substring(4);
    }
  } else if (typeof params === "string") {
    var match = params.match(/[?&]id=(\d+)/);
    if (match) vodId = match[1];
  }
  
  if (!vodId) {
    logError("无法提取 vod_id，参数: " + JSON.stringify(params));
    throw new Error("无法解析视频ID");
  }
  
  if (!apiHost) apiHost = DEFAULT_API_HOST;
  apiHost = apiHost.replace(/\/$/, "");
  
  return getDetail(apiHost, vodId).then(function(detail) {
    if (!detail || !detail.urls || !detail.urls.length) {
      throw new Error("获取详情失败或无播放源");
    }
    ...
    if (detail.urls.length === 1) {
      ...
      if (!onlyTitle.match(/第\d+集/) && onlyTitle.indexOf("集") === -1) {
        isMovie = true;
      }
    }
    ...
    for (var i = 0; i < detail.urls.length; i++) {
      ...
      if (isMovie) {
        ...
        if (epNum !== null) {
          epDisplayTitle = title + " 第" + epNum + "集";
        } else if (detail.urls.length > 1) {
          epDisplayTitle = title + " 第" + (i+1) + "集";
        } else {
          epDisplayTitle = title + " " + (rawTitle || "播放");
        }
      }
    ...
    return {
      ...
      posterPath: detail.pic || "",
      backdropPath: detail.pic || "",
      mediaType: mediaType,
...
WidgetMetadata = {
  ...
  description: "欧乐影视、分类浏览（电影/剧集/综艺/动漫/短剧）",
  author: "MoYan",
  globalParams: [
    {
      name: "ApiHost",
  ...
  modules: [
      ...
      params: [
        ...
        },
        {
          ...
          enumOptions: [
            ...
            { title: "高分电影", value: "score" },
            { title: "最新电影", value: "update" },
            { title: "最近添加", value: "desc" }
          ]
        },
    ...
    },
    {
      ...
      params: [
        ...
        },
        {
          ...
          value: "hot",
          enumOptions: [
            { title: "热门剧集", value: "hot" },
            { title: "高分剧集", value: "score" },
            { title: "最新剧集", value: "update" },
            { title: "最近添加", value: "desc" }
          ]
        },
        { name: "page", title: "页码", type: "page", startPage: 1 }
    ...
    },
    {
      ...
      params: [
        ...
        },
        {
          ...
          value: "hot",
          enumOptions: [
            { title: "热门综艺", value: "hot" },
            { title: "高分综艺", value: "score" },
            { title: "最新综艺", value: "update" },
            { title: "最近添加", value: "desc" }
          ]
        },
        { name: "page", title: "页码", type: "page", startPage: 1 }
    ...
    },
    {
      ...
      params: [
        ...
        },
        {
          ...
          value: "hot",
          enumOptions: [
            { title: "热门动漫", value: "hot" },
            { title: "高分动漫", value: "score" },
            { title: "最新动漫", value: "update" },
            { title: "最近添加", value: "desc" }
          ]
        },
        { name: "page", title: "页码", type: "page", startPage: 1 }
    ...
    },
    {
      id: "ole_short",
      title: "短剧",
      description: "浏览欧乐短剧库，可切换地区与榜单",
      functionName: "loadShortList",
      type: "video",
      cacheDuration: 3600,
      params: [
        {
        ...
        },
        {
          ...
          value: "hot",
          enumOptions: [
            { title: "热门短剧", value: "hot" },
            { title: "高分短剧", value: "score" },
            { title: "最新短剧", value: "update" },
            { title: "最近添加", value: "desc" }
          ]
        },
        { name: "page", title: "页码", type: "page", startPage: 1 }
